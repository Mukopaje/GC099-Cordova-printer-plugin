...
{
  protected DFDF4 m_deviceParameters;
  private DFDF1.DATA_TYPE m_expectedReceiveType;
  protected EEE11A m_callbackInfo = new EEE11A();
  protected A2 m_callback = null;
...
//  private Vector<Byte> m_imageBuffer = new Vector(1000000, 1000000);
  ...
  private DFDF1.RECEIVESTATE m_receiveState;
  private DFDF1.RECEIVESUBSTATE m_receiveSubState;
...
//  private ReentrantLock m_imageBufferLock = new ReentrantLock();
  ...
  protected DFDF1.ERROR_CODE m_Error = DFDF1.ERROR_CODE.SUCCESS;

  abstract DFDF1.ERROR_CODE openPort();

  abstract DFDF1.ERROR_CODE closePort();

  abstract boolean isPortOpen();

  abstract DFDF1.ERROR_CODE writeData(Vector<Byte> paramVector);
  

  protected abstract DFDF1.ERROR_CODE writeDataImmediately(Vector<Byte> paramVector);

  FFF(DFDF4 parameters)
  {
    this.m_deviceParameters = parameters;
    this.m_expectedReceiveType = DFDF1.DATA_TYPE.GENERAL;
    ...
    this.m_receiveState = DFDF1.RECEIVESTATE.RSTATE_SINGLE;
    this.m_receiveSubState = DFDF1.RECEIVESUBSTATE.RSUBSTATE_INITSTATE;
  ...
  private Vector<Byte> getData(Vector<Byte> source, ReentrantLock sourceLock)
  {
    ...
    if (source.size() > 0)
    {
      ...
      {
        CCCC.d("Port.getData", "Exception occured: " + e.getMessage());
      }
      finally
      {
        sourceLock.unlock();
  ...
  private byte[] getDataBytes(Vector<Byte> source, ReentrantLock sourceLock)
  {
    ...
    if (source.size() > 0)
    {
      ...
      {
        CCCC.d("Port.getDataBytes", "Exception occured: " + e.getMessage());
      }
      finally
      {
        sourceLock.unlock();
  ...
  public DFDF1.ERROR_CODE getError()
  {
    return this.m_Error;
  }
...
//  }
  ...
  protected void parseOutgoingData(Vector<Byte> data)
  {
    ...
    if ((data.size() > 2) && (((Byte)data.get(0)).byteValue() == DFDF1.ASCII_CONTROL_CODE.DLE.getASCIIValue()) && (((Byte)data.get(1)).byteValue() == DFDF1.ASCII_CONTROL_CODE.EOT.getASCIIValue()))
    {
...
//    }
  ...
  protected void afterCallbackAction(DFDF1.DATA_TYPE dataType)
  {
...
//    }
  ...
  protected DFDF1.ERROR_CODE saveData(Vector<Byte> receivedData)
  {
    byte oneByte = 0;
    int dataCounter = 0;
    int dataSize = 0;
    ...
    byte bACK = DFDF1.ASCII_CONTROL_CODE.ACK.getASCIIValue();
    ...
    while (receivedData.size() > 0)
    {
      ...
      case RSTATE_SINGLE:
        ...
        if ((oneByte & 0x93) == 18)
        {
          ...
          this.m_expectedReceiveType = DFDF1.DATA_TYPE.GENERAL;
          this.m_callbackInfo.ReceivedDataType = DFDF1.DATA_TYPE.DEVICESTATUS;
        }
        else if ((oneByte & 0x93) == 16)
        {
          ...
          this.m_receiveState = DFDF1.RECEIVESTATE.RSTATE_ASB;
        }
        else if (oneByte == 83)
        {
          this.m_dataBlockSize = 0;
          this.m_receiveState = DFDF1.RECEIVESTATE.RSTATE_BLOCK_BINARY;
        }
...
//        }
        else
        {
          ...
          this.m_receiveState = DFDF1.RECEIVESTATE.RSTATE_SINGLE;
          this.m_callbackInfo.ReceivedDataType = DFDF1.DATA_TYPE.GENERAL;
        }
      ...
      case RSTATE_ASB:
        ...
        {
          this.m_receiveState = DFDF1.RECEIVESTATE.RSTATE_SINGLE;
          this.m_callbackInfo.ReceivedDataType = DFDF1.DATA_TYPE.ASB;
        }
        break;
...
//          }
          else if (this.m_receiveCounter > 2L)
          {
            ...
            case RSUBSTATE_FILEINFO:
              this.m_callbackInfo.ReceivedDataType = DFDF1.DATA_TYPE.NOTHING;
              ...
              if (!blockEndFlag.booleanValue())
                continue;
              this.m_receiveSubState = DFDF1.RECEIVESUBSTATE.RSUBSTATE_SIZEINFO;
              this.m_receiveState = DFDF1.RECEIVESTATE.RSTATE_SINGLE;
            ...
            case RSUBSTATE_SIZEINFO:
              ...
              if (!blockEndFlag.booleanValue())
                continue;
              this.m_receiveSubState = DFDF1.RECEIVESUBSTATE.RSUBSTATE_IMAGEDATA;
              this.m_receiveState = DFDF1.RECEIVESTATE.RSTATE_SINGLE;
              writeDataImmediately(v_ACK);
      ...
      case RSTATE_BLOCK_BINARY:
        if (this.m_receiveCounter == 1L)
        {
          ...
          if (this.m_identifier == 32)
          {
            ...
            this.m_sizeInfoBlock.clear();
//            this.m_imageDataBlock.clear();
            this.m_receiveSubState = DFDF1.RECEIVESUBSTATE.RSUBSTATE_FILEINFO;
          }
...
//        break;
    ...
    {
    case DEVICESTATUS:
      CCCC.d("Port", "m_RealtimeStatusReady=true");
      this.m_RealtimeStatusReady = Boolean.valueOf(true);
      break;
//    case RESERVED:
//      debug.d("Port", "m_MICRDataReady=true");
//      this.m_MICRDataReady = Boolean.valueOf(true);
//      break;
//    case IMAGE:
//      debug.d("Port", "m_ImageDataReady=true");
//      this.m_ImageDataReady = Boolean.valueOf(true);
//      break;
    case ASB:
      CCCC.d("Port", "ASB data ready");
      break;
    case GENERAL:
      CCCC.d("Port", "GENERAL data ready");
      break;
    }
